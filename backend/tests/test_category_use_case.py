...
def test_subscribe_to_category():
    mock_repo = MagicMock()
    use_case = ManageCategoryUseCase(
        questions_repository=mock_repo, auth_repository=MagicMock()
    )

    use_case.subscribe_to_category_or_sub_category(
        category_id=1, sub_category_id=None, user_id=1
    )
...
def test_subscribe_to_sub_category():
    ...
    use_case.subscribe_to_category_or_sub_category(
        category_id=None, sub_category_id=2, user_id=1
    )
